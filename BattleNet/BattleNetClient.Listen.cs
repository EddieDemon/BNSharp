    ...
    partial class BattleNetClient
    {
        ...
        partial void InitializeParseDictionaries()
        {
            ...
            m_packetToParserMap = new Dictionary<BncsPacketId, ParseCallback>()
            {
                ...
                { BncsPacketId.SetEmail, HandleSetMail } // NOTE: Set handle for set mail request.
            };
        }
        ...
        // NOTE: Handles the setmail request.
        private void HandleSetMail(ParseData data)
        {
            blnSetMail = true;
            OnSetMailRequest(new SetMailRequestEventArgs());
        }
        ...
        private void HandleGetChannelList(ParseData data)
        {
            ...
            if (!m_firstChannelList)
            {
                ...
                // NOTE: Check if the home channel is null. If not, go to it's home channel.
                if (!string.IsNullOrWhiteSpace(m_settings.HomeChannel))
                    pckJoinChan.InsertInt32((int)ChannelJoinFlags.ForcedJoin);
                else if (m_settings.Client.Equals("D2DV", StringComparison.Ordinal)
        ...
        private void HandleAuthInfo(ParseData data)
        {
            try
            {
                ...
                // Note: Check if there's a curstom version set, if so, send that instead of the found version.
                if (m_settings.CustomExeVersion == null)
                    pck0x51.Insert(exeVer);
                else
                    pck0x51.Insert(m_settings.CustomExeVersion);
        ...
        private void HandleAuthAccountLogon(ParseData data)
        {
            ...
            if (status == 0)
            {
                // NOTE: Break the login sequence. Use old login instead of sending LoginProof.
